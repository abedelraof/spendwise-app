...
telegram_bucket_sessions (chat_id PRIMARY KEY → telegram_links, state JSONB, updated_at)
-- the /assign bucketing stepper's cursor state { date, label, txns[], cursor, sel[] };
-- deliberately SEPARATE from telegram_sessions so on('text') never treats it as a pending expense parse
```
...
- **Expense entry**: any text message from a linked chat with no pending session → `aiService.parseExpenses()`; with a pending session (`telegram_sessions`) → `aiService.reviseExpenses()` (sends the pending list + the new message to Claude so it can apply corrections like "actually lunch was 150"). Before showing the expense list, `detectNewCategories()` diffs the parsed category/subcategory names against the user's existing ones (`categoryModel.findByUser`); if any don't exist yet, the bot instead replies with the suggested new categories and a `[✅ Confirm] [❌ Cancel]` keyboard (`pending_new_categories` stored alongside `pending_expenses` in `telegram_sessions`). Confirming there calls `categoryService.matchOrCreateCategory()` for each suggestion, then shows the normal expense-list confirm; cancelling calls `remapToExistingCategories()` to fall back each affected expense onto an existing category (`Other` if present, else the user's first category) with no DB writes, then shows the expense list unchanged otherwise. Either way, the final expense list reply has the inline `[✅ Confirm] [❌ Cancel]` keyboard — confirm calls `expenseService.createExpenses()` (the same function `POST /api/expenses` uses) and stores the new expense IDs in `telegram_links.last_expense_ids`.
...
- **Pull commands** (all require a linked account): `/stats [today|yesterday|week|month]`, `/report [today|yesterday|week|month]` (a rendered PNG report card), `/reportpdf [today|yesterday|week|month]` (the same card as a PDF — see "Report Images & PDF" below), `/budgets`, `/recent`, `/goals`, `/networth`, `/currency <amount> <from> [to] <to>`, `/undo` (deletes the exact batch in `last_expense_ids`, then clears it — one-shot, not a general history undo), `/ask <question>` (same context-builder as Finance Chat, via `expenseService.getFinanceContext`), `/assign [today|yesterday|YYYY-MM-DD]` (bucket a day's expenses one at a time — see "Bucket assignment via Telegram" below), `/help`.
...
### Report Images & PDF (`/report`, `/reportpdf`)
- `/report [today|yesterday|week|month]` sends the card as a **PNG image**; `/reportpdf [today|yesterday|week|month]` sends the **same card as a single-page PDF** (crisp, selectable vector text). Over HTTP: `GET /api/reports/report.png?period=…` and `GET /api/reports/report.pdf?period=…`. Both cover: header + 4 KPI tiles, a spending-trend line, a category breakdown, and budgets + net worth.
- Three layers, deliberately separable: `reportService.js` (assembles data from existing services only — no new queries; `generateReportPng`/`generateReportPdf`), `reportTemplate.js` (pure `data → HTML string`, no I/O), `renderService.js` (the only file that imports Puppeteer; `renderHtmlToPng`/`renderHtmlToPdf` share one `withPage` browser path). Iterate on the design via `report.png?...&debug=html` in a browser rather than rebuilding a container.
- The bot's `/report` and `/reportpdf` handlers share `runReport(ctx, { deliver, … })` — same linked-user guard, period parsing, refreshing chat action, and fallback; they differ only in the `deliver` callback (photo vs document).
- Chromium is a lazily-launched **singleton** with crash recovery (`disconnected` → relaunch next call), a 5-minute idle shutdown (`REPORT_BROWSER_IDLE_MS` overrides, used by tests), and renders serialized one page at a time to bound memory. `server.js` closes it on SIGTERM/SIGINT so nodemon restarts don't leak browsers.
- **`page.pdf()` returns a `Uint8Array`, not a `Buffer`** (unlike `screenshot()`); `renderHtmlToPdf` coerces with `Buffer.from` because Express `send()` and Telegraf both silently mishandle a bare Uint8Array. The PDF paper is sized to the `#report` element's own box (so it's one page, not A4-paginated) and `printBackground: true` is mandatory or the dark theme prints white with invisible text.
- If rendering fails for any reason, both commands fall back to the `/stats` text via the shared `buildStatsText(user, period)` — the feature degrades to existing behaviour rather than erroring.
- Budgets and net worth have no range-scoped query, so they are **always month-to-date** regardless of the selected period, and the card labels them that way.
...
**Bucket assignment via Telegram — `/assign [today|yesterday|YYYY-MM-DD]`** (`telegramBotService.js`): a button-driven stepper that walks every expense on the chosen date one at a time. Each transaction renders as a message with one toggle button per bucket (✅ when selected, pre-checked from the expense's existing `bucket_ids`) plus Done / Skip / Cancel. **Done** calls `bucketModel.setExpenseBuckets` and advances; **Skip** advances unchanged; the final step edits a summary and clears state. Cursor state lives in `telegram_bucket_sessions` (its own table, so the `on('text')` expense-parse resume logic never touches it), and per-button `callback_data` carries a single bucket id (`ab:<id>`, well under Telegram's 64-byte cap) — the accumulated selection is in the session JSONB. This introduced the first **regex `instance.action(/^ab:(\d+)$/)`** handler in the file (standard Telegraf); toggles/advances use `safeEdit` to swallow Telegram's benign "message is not modified" error. The same stepper is also **chained automatically after `confirm_expenses`**: once new expenses are recorded, if the user has any buckets the confirmation message turns straight into the assign stepper over the just-created expenses (seeded from `created[]`, with optional `state.intro`/`state.doneMsg` carrying the "Recorded N…" text); users with no buckets just get the plain confirmation.
...
| GET | `/report.png?period=month` | The `/report` card as a PNG. `&debug=html` (non-production) returns the raw HTML instead |
| GET | `/report.pdf?period=month` | The same card as a single-page PDF (the `/reportpdf` output) |
...
21. **`getMonthRange` drops the last day of 31-day months**: `new Date(year, month, 0)` builds a local-midnight date and `.toISOString()` then shifts it back a day in any positive-UTC-offset timezone — so July resolves to `2026-07-01 … 2026-07-30`. Every month-scoped query (dashboard stats, budgets, `/stats`, `/report`) inherits this and silently excludes expenses dated the 31st. Pre-existing and not specific to reports; fix it in `dateUtils.js` rather than working around it per-caller.
