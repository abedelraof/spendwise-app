...
- **Pull commands** (all require a linked account): `/stats [today|yesterday|week|month]`, `/report [today|yesterday|week|month]` (a rendered PNG report card — see "Report Images" below), `/budgets`, `/recent`, `/goals`, `/networth`, `/currency <amount> <from> [to] <to>`, `/undo` (deletes the exact batch in `last_expense_ids`, then clears it — one-shot, not a general history undo), `/ask <question>` (same context-builder as Finance Chat, via `expenseService.getFinanceContext`), `/help`.
...
- `/report [today|yesterday|week|month]` in the bot, and `GET /api/reports/report.png?period=…` over HTTP, render the same comprehensive report card: header + 4 KPI tiles, a spending-trend line, a category breakdown, and budgets + net worth.
- Three layers, deliberately separable: `reportService.js` (assembles data from existing services only — no new queries), `reportTemplate.js` (pure `data → HTML string`, no I/O), `renderService.js` (the only file that imports Puppeteer). Iterate on the design via `?debug=html` in a browser rather than rebuilding a container.
...
- If rendering fails for any reason, `/report` falls back to the `/stats` text via the shared `buildStatsText(user, period)` — the feature degrades to existing behaviour rather than erroring.
