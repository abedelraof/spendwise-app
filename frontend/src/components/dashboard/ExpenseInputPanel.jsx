...
import { Sparkles, Loader2, PenLine, Plus, Check, X, Zap, LayoutList } from 'lucide-react';
...
/* ── Style picker metadata ─────────────────────────────────────────── */
const STYLES = [
  { id: 1, label: 'Aurora',    dot: 'bg-violet-500'  },
  { id: 2, label: 'Slate',     dot: 'bg-cyan-500'    },
  { id: 3, label: 'Sunrise',   dot: 'bg-orange-400'  },
  { id: 4, label: 'Terminal',  dot: 'bg-emerald-500' },
];
...
export default function ExpenseInputPanel({
  ...
  function chooseStyle(n) { setStyleId(n); localStorage.setItem('eb_panel_style', String(n)); setPicker(false); }
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5">
        {picker && (
          <div className="flex items-center gap-0.5 bg-white dark:bg-slate-800 rounded-xl shadow-xl ring-1 ring-gray-200 dark:ring-slate-700 px-2 py-1.5 mr-1">
            {STYLES.map(s => (
              <button key={s.id} onClick={() => chooseStyle(s.id)} title={s.label}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-semibold transition-all ${
                  styleId === s.id
                    ? 'bg-gray-100 dark:bg-slate-700 text-gray-900 dark:text-white'
                    : 'text-gray-400 hover:text-gray-700 dark:hover:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/60'
                }`}>
        ...
        <button onClick={() => setPicker(p => !p)} title="Switch style"
          className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all shadow-sm ${
            picker
              ? 'bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-slate-300'
              : 'bg-white/80 dark:bg-slate-700/80 text-gray-400 hover:text-gray-700 dark:hover:text-slate-300 backdrop-blur-sm ring-1 ring-gray-200/50 dark:ring-slate-600/50'
          }`}>
          <LayoutList size={13} />
        </button>
      </div>
